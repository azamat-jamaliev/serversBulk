...
func NewMainPageController(version string, appObj *tview.Application, lastItemSelectedHandlerFunc func()) (*PageController, *tview.Flex, *tview.Grid) {
	controller := NewPageController(appObj, lastItemSelectedHandlerFunc)
	controller.header = newPrimitive(fmt.Sprintf("!!! SeBulk %s !!! \nworks when GrayLog or Ansible is not available", version))
